...
## GA Implementation on Restaurant Problem

[Let's run a genetic algorithm first, then see how it works:]{.highlight}
...
print(f"GA found solution: €{ga_best_cost:.0f}")
print(f"Improvement over greedy: {100*(greedy_cost_value-ga_best_cost)/greedy_cost_value:.1f}%")
```

[Now let's understand HOW it works by examining each stage...]{.highlight}

## The Genetic Process: Overview
...
[Let's see each stage in detail with our restaurant problem...]{.highlight}
...
np.random.seed(42)
population_size = 20
costs = np.random.randint(1400, 1900, population_size)
costs = sorted(costs)
...
ax.text(costs[0] + 30, 0, f'Best: €{costs[0]}', va='center', fontsize=10, fontweight='bold', color='green')
ax.text(costs[-1] - 30, population_size-1, f'Worst: €{costs[-1]}', va='center', fontsize=10, fontweight='bold', color='red', ha='right')
...
[Better schedules have higher chance of being selected, but not guaranteed!]{.highlight}
...
# Parent 1
parent1_shifts = ['Fri_Din', 'Fri_Late', 'Sat_Lun', 'Sat_Din', 'Sun_Lun', 'Sun_Din']
parent1_servers = [['E1','J6'], ['J2','J4'], ['E3','J1'], ['E2','J5'], ['J7','J8'], ['E4','J3']]
parent1_cost = 1450

y_start = 9
ax.text(1, y_start, 'Parent 1', fontsize=14, fontweight='bold', 
        bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], edgecolor='black', linewidth=2))
ax.text(9, y_start, f'€{parent1_cost}', fontsize=12, fontweight='bold')

for i, (shift, servers) in enumerate(zip(parent1_shifts, parent1_servers)):
    y = y_start - 0.7 - i*0.6
    ax.text(1, y, f'{shift}:', fontsize=10, ha='left')
    ax.text(3, y, f'{servers[0]}, {servers[1]}', fontsize=10, 
            bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.5, pad=0.3))

# Parent 2
parent2_servers = [['J6','E1'], ['J3','J5'], ['E2','J7'], ['E3','E4'], ['J1','J2'], ['J8','J4']]
parent2_cost = 1480

y_start = 9
ax.text(5.5, y_start, 'Parent 2', fontsize=14, fontweight='bold',
        bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["twoLight"], edgecolor='black', linewidth=2))
ax.text(9, y_start, f'€{parent2_cost}', fontsize=12, fontweight='bold')

for i, (shift, servers) in enumerate(zip(parent1_shifts, parent2_servers)):
    y = y_start - 0.7 - i*0.6
    ax.text(5.5, y, f'{shift}:', fontsize=10, ha='left')
    ax.text(7.5, y, f'{servers[0]}, {servers[1]}', fontsize=10,
            bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["twoLight"], alpha=0.5, pad=0.3))

# Arrow down
ax.annotate('', xy=(5, 3.5), xytext=(5, 4.5),
            arrowprops=dict(arrowstyle='->', lw=3, color='black'))
ax.text(5, 4, 'Order Crossover', ha='center', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))

# Child (take first 3 shifts from parent 1, rest from parent 2)
child_servers = parent1_servers[:3] + parent2_servers[3:]
child_cost = "???"

y_start = 2.5
ax.text(3, y_start, 'Child (Offspring)', fontsize=14, fontweight='bold',
        bbox=dict(boxstyle='round', facecolor='lightgreen', edgecolor='black', linewidth=2))
ax.text(9, y_start, f'Cost: {child_cost}', fontsize=12, fontweight='bold')

for i, (shift, servers) in enumerate(zip(parent1_shifts, child_servers)):
    y = y_start - 0.7 - i*0.6
    # Color based on which parent
    color = BRAND_COLORS["oneLight"] if i < 3 else BRAND_COLORS["twoLight"]
    ax.text(3, y, f'{shift}:', fontsize=10, ha='left')
    ax.text(5, y, f'{servers[0]}, {servers[1]}', fontsize=10,
            bbox=dict(boxstyle='round', facecolor=color, alpha=0.6, pad=0.3))

# Add legend
ax.text(0.5, -1, 'From Parent 1', fontsize=10, 
        bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.6))
ax.text(3, -1, 'From Parent 2', fontsize=10,
        bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["twoLight"], alpha=0.6))

plt.tight_layout()
plt.show()
```

[Inherit good features from both parents!]{.highlight}
...
# Left: Before mutation
ax = axes[0]
ax.set_xlim(0, 10)
ax.set_ylim(0, 10)
ax.axis('off')

before_servers = [['E1','J6'], ['J2','J4'], ['E3','J1'], ['E2','J5'], ['J7','J8'], ['E4','J3']]
shifts = ['Fri_Din', 'Fri_Late', 'Sat_Lun', 'Sat_Din', 'Sun_Lun', 'Sun_Din']

ax.text(5, 9.5, 'Before Mutation', fontsize=15, fontweight='bold', ha='center')

for i, (shift, servers) in enumerate(zip(shifts, before_servers)):
    y = 8.5 - i*1.2
    ax.text(2, y, f'{shift}:', fontsize=11, ha='left', fontweight='bold')
    
    # Highlight the two servers that will be swapped
    if i == 1:  # Fri_Late
        ax.text(5, y, f'{servers[0]}', fontsize=11, 
                bbox=dict(boxstyle='round', facecolor='yellow', edgecolor='red', linewidth=3, pad=0.4))
        ax.text(6.5, y, f'{servers[1]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.5, pad=0.4))
    elif i == 4:  # Sun_Lun
        ax.text(5, y, f'{servers[0]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor='yellow', edgecolor='red', linewidth=3, pad=0.4))
        ax.text(6.5, y, f'{servers[1]}', fontsize=11,
        ...
        ax.text(5, y, f'{servers[0]}, {servers[1]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.5, pad=0.4))

ax.text(5, 1, 'Mutation point: Swap J2 ↔ J7', fontsize=12, ha='center', color='red', fontweight='bold',
        bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7, pad=0.5))

# Right: After mutation
ax = axes[1]
ax.set_xlim(0, 10)
ax.set_ylim(0, 10)
ax.axis('off')

after_servers = [['E1','J6'], ['J7','J4'], ['E3','J1'], ['E2','J5'], ['J2','J8'], ['E4','J3']]

ax.text(5, 9.5, 'After Mutation', fontsize=15, fontweight='bold', ha='center')

for i, (shift, servers) in enumerate(zip(shifts, after_servers)):
    y = 8.5 - i*1.2
    ax.text(2, y, f'{shift}:', fontsize=11, ha='left', fontweight='bold')
    
    # Highlight the swapped servers
    if i == 1:  # Fri_Late - now has J7
        ax.text(5, y, f'{servers[0]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor='lightgreen', edgecolor='green', linewidth=3, pad=0.4))
        ax.text(6.5, y, f'{servers[1]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.5, pad=0.4))
    elif i == 4:  # Sun_Lun - now has J2
        ax.text(5, y, f'{servers[0]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor='lightgreen', edgecolor='green', linewidth=3, pad=0.4))
        ax.text(6.5, y, f'{servers[1]}', fontsize=11,
                bbox=dict(boxstyle='round', facecolor=BRAND_COLORS["oneLight"], alpha=0.5, pad=0.4))
...
ax.text(5, 1, f'Mutation rate: 20%\n(happens 1 in 5 offspring)', fontsize=11, ha='center', 
        style='italic', bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.5, pad=0.5))

plt.tight_layout()
plt.show()
```

[Mutation prevents premature convergence to local optima!]{.highlight}
...
# Use actual GA data from earlier (don't overwrite SA variables!)
generations = list(range(len(ga_best_hist)))

ax.plot(generations, ga_best_hist, color='green', linewidth=4, label='Best in Population', marker='o', markersize=8)
ax.plot(generations, ga_avg_hist, color='blue', linewidth=3, label='Population Average', marker='s', markersize=6, linestyle='--')

# Add greedy baseline
ax.axhline(y=greedy_cost_value, color='gray', linestyle=':', linewidth=2, label=f'Greedy (€{greedy_cost_value:.0f})', alpha=0.6)

# Annotate phases
ax.axvspan(0, 30, alpha=0.1, color='red', label='Exploration Phase')
ax.axvspan(30, 70, alpha=0.1, color='orange')
ax.axvspan(70, len(generations), alpha=0.1, color='green')

ax.text(15, max(ga_avg_hist)*0.95, 'EXPLORATION\nHigh diversity', ha='center', fontsize=11, fontweight='bold', color='red')
ax.text(50, max(ga_avg_hist)*0.95, 'REFINEMENT\nConverging', ha='center', fontsize=11, fontweight='bold', color='orange')
ax.text(85, max(ga_avg_hist)*0.95, 'EXPLOITATION\nFine-tuning', ha='center', fontsize=11, fontweight='bold', color='green')
...
[Tabu = Smart algorithm that learns from history!]{.highlight}
...
ax.axhline(y=best_cost_sa, color=BRAND_COLORS["twoDark"], linestyle='--', linewidth=2,
           label=f'SA Best (€{best_cost_sa:.0f})', alpha=0.6)
...
print(f"{'='*60}")
print(f"Greedy:      €{greedy_cost_value:,.0f}")
print(f"SA:          €{best_cost_sa:,.0f}")
print(f"GA:          €{ga_best_cost:,.0f}")
print(f"Tabu Search: €{tabu_best_cost:,.0f}")
```

[Tabu Search's memory prevents revisiting bad solutions!]{.highlight}
