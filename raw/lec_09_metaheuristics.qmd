...
## **[Client Briefing: La Étoile Restaurant]{.invert-font}** {background-image="https://unsplash.com/photos/Gg5tmm48v0w/download?ixid=M3wxMjA3fDB8MXxzZWFyY2h8Mnx8bWljaGVsaW5lJTIwc3RhciUyMHJlc3RhdXJhbnR8ZW58MHx8fHwxNzMxNTI0MTAxfDA&force=true&w=2400" background-size="cover"}
...
[How do we minimize the damage while maintaining service quality?]{.question}
...
::: {.incremental}
1. **Understand** why local search gets trapped and needs help
2. **Master** four powerful metaheuristics:
   - Simulated Annealing (SA)
   - Genetic Algorithms (GA)
   - Tabu Search
   - Ant Colony Optimization (ACO)
3. **Implement** working code for each metaheuristic
4. **Choose** the right metaheuristic for your problem
5. **Avoid** common implementation pitfalls
6. **Apply** metaheuristics to real-world problems
:::

. . .

[Let's escape those local optima!]{.highlight}

## The Optimization Challenge: Hiking in Fog
...
- **Goal**: Find optimal solution (max profit/min cost)
- **Challenge**: Can't evaluate all possibilities
- **Decisions**: Staff assignments, routes, investments
- **Quality**: Current profit/cost
- **Problem**: Local optimum = trapped in suboptimal solution
:::
:::

. . .

[This metaphor will guide us through all metaheuristics!]{.highlight}
...
y = 3 + np.random.randn(100) * 0.5
ax.plot(x, y, 'gray', linewidth=2, alpha=0.5)
ax.scatter([5], [3], color='red', s=200, marker='X', zorder=5)
ax.set_title('Random Search\n(No Strategy)', fontsize=12, fontweight='bold')
ax.set_xlabel('Solution Space')
ax.set_ylabel('Quality')
ax.text(5, 2, 'Random\nGuess', ha='center', fontsize=10)
...
ax = axes[0]
shifts = range(1, 7)
possibilities = [comb(18, 3) ** i for i in shifts]
ax.semilogy(shifts, possibilities, 'o-', color=BRAND_COLORS["twoDark"], linewidth=3, markersize=10)
ax.fill_between(shifts, 1, possibilities, alpha=0.3, color=BRAND_COLORS["twoLight"])
ax.set_xlabel('Number of Shifts', fontsize=12)
ax.set_ylabel('Total Schedules (log scale)', fontsize=12)
ax.set_title('Combinatorial Explosion', fontsize=14, fontweight='bold')
ax.grid(True, alpha=0.3)

# Add annotations
ax.text(6, possibilities[-1], f'{possibilities[-1]:,.0f}\nschedules!', 
...
[With 139 billion possibilities, we need smart exploration!]{.highlight}
...
colors = [BRAND_COLORS["oneLight"], 'red', BRAND_COLORS["threeDark"]]
...
ax.scatter([greedy_x], [greedy_y], color='orange', s=200, marker='o', zorder=5, label='Greedy Start')
ax.scatter([local_x], [local_y], color='red', s=200, marker='*', zorder=5, label='Local Optimum')
ax.scatter([global_x], [global_y], color='green', s=200, marker='*', zorder=5, label='Global Optimum')

# Show local search path
ax.arrow(greedy_x, greedy_y, local_x-greedy_x-0.1, local_y-greedy_y+5,
         head_width=0.2, head_length=5, fc='orange', ec='orange', alpha=0.7)

# Show barrier
ax.axvspan(local_x+0.5, global_x-0.5, alpha=0.2, color='gray')
...
# Draw center
circle = Circle(center, 0.08, color='red', alpha=0.7)
ax.add_patch(circle)
...
for i, (x, y) in enumerate(neighbors):
    ...
    circle = Circle((x, y), 0.06, color='lightgray', alpha=0.7)
    ax.add_patch(circle)
...
for ax, T, title in zip(axes, temperatures, titles):
    ...
    ax.plot(delta_costs, probabilities, linewidth=3, color=BRAND_COLORS["oneDark"])
    ax.fill_between(delta_costs, 0, probabilities, alpha=0.3, color=BRAND_COLORS["oneLight"])
...
# CONCEPT: The Restaurant Scheduling Problem
"""
What we're trying to solve:
- 18 servers (6 experienced @ €80, 12 junior @ €50)
- 6 shifts (each needs 3 servers)
- Problem: Need at least 1 experienced per shift
- Penalty: €500 if shift has no experienced server
...
def restaurant_cost_concept(schedule):
    """
    Calculate cost (simplified concept)
    """
    cost = 0
    for shift in schedule:
        cost += count_experienced(shift) * 80
        cost += count_junior(shift) * 50
        if no_experienced_in(shift):
            cost += 500  # Penalty!
    return cost
...
def make_neighbor(schedule):
    ...
    This is how we explore nearby solutions
    """
    # Pick two random shifts and swap one server from each
    return schedule_with_one_swap

print("The challenge: Balance cost vs coverage across all shifts!")
```
...
# Top: Cost over time
ax = axes[0]
ax.plot(iterations, greedy_cost, 'r--', linewidth=2, label='Greedy (stuck)', alpha=0.7)
ax.plot(iterations, sa_costs, 'b-', linewidth=1, alpha=0.5, label='SA Current')
ax.plot(iterations, best_costs, 'g-', linewidth=3, label='SA Best')
...
ax.axvspan(0, 20, alpha=0.1, color='red', label='Exploration')
ax.axvspan(20, 60, alpha=0.1, color='orange', label='Transition')
ax.axvspan(60, 100, alpha=0.1, color='blue', label='Exploitation')
...
## Common SA Mistakes (Avoid These!)
...
- Fix: Cool slowly (multiply by 0.95, not 0.5)

**Mistake #3: Forgetting the Best**
- Current solution might get worse near the end
- Fix: Always track the best ever found

**Mistake #4: Not Understanding "Why"**
- SA works because it escapes local traps
- Without bad move acceptance, it's just hill climbing!
...
ax.text(4, 1750, 'Tournament\n(pick best)', ha='center', color='green', fontweight='bold')
...
ax.plot(iterations, sa_best, 'b-', linewidth=3, label='SA (single solution)')
ax.plot(iterations, ga_best, 'g-', linewidth=3, label='GA (best in population)')
ax.plot(iterations, ga_avg, 'g--', linewidth=1, alpha=0.5, label='GA (population average)')
ax.set_xlabel('Iteration', fontsize=12)
...
ax.plot(sa_x, sa_y, 'b-', linewidth=2, alpha=0.7, label='SA path')
ax.scatter(sa_x[-1], sa_y[-1], color='blue', s=100, marker='*')

# GA population (multiple points converging)
ga_pop_x = np.random.uniform(0, 10, 20)
ga_pop_y = np.random.uniform(0, 10, 20)
ax.scatter(ga_pop_x, ga_pop_y, color='green', s=50, alpha=0.5, label='GA population')
# Show convergence
ax.scatter([7], [7], color='green', s=200, marker='*', label='GA convergence')
...
# CONCEPT: Tabu Search - Learning from Mistakes
def tabu_search_concept():
    ...
    while not done:
        ...
        # Remember this move (add to "never again" list)
...
# Business Parallel:
# Like a company keeping a "lessons learned" database
# "We tried that marketing campaign in 2022 - it failed"
# "That supplier was unreliable - avoid for 2 years"
# This institutional memory prevents repeating mistakes!

print("Tabu = Smart organizations that learn from history!")
```
    ...
    for i in range(n_nodes):
        for j in range(i+1, n_nodes):
            ...
            ax.plot([node_x[i], node_x[j]], [node_y[i], node_y[j]], 
                   'gray', alpha=alpha, linewidth=width)
...
3. **Tune Parameters**: Use these rules of thumb:
...
**Resources**:
- 18 servers (6 experienced @ €80, 12 junior @ €50)
- 6 shifts (3 servers each, need 1+ experienced)
- Penalty: €500 per shift without experienced server
...
[Remember: Document your approach and reasoning!]{.highlight}
...
|-----------|----------|---------|
| Need answer in < 1 min | Greedy + Local Search | Fast and often good enough |
| Stuck in local optimum | Simulated Annealing | Controlled chaos escapes traps |
| Complex with many variables | Genetic Algorithm | Population explores broadly |
| Keep revisiting same bad solutions | Tabu Search | Memory prevents cycling |
| Routing/network problem | Ant Colony | Pheromones optimize paths |
...
np.random.seed(456)
colors = [BRAND_COLORS["oneDark"], BRAND_COLORS["twoDark"], 'green']
...
- Variables: 18 staff to 6 shifts
- Representation: 2D array [staff][shift]

HARD CONSTRAINTS (penalty: 1000):
- Coverage: Min 3 staff per shift
- Skill: 1+ experienced per shift
- Legal: Max 5 shifts per person

SOFT CONSTRAINTS (penalty: 10-100):
- Preferences: +20 per violation
- Fairness: +10 per imbalance
- Fatigue: +50 consecutive shifts

ALGORITHM: Implement SA with:
- Neighbor: Swap two assignments
- Initial temp: 1000, cooling: 0.95
...
colors = ['gray', BRAND_COLORS["oneLight"], BRAND_COLORS["twoLight"], 
          BRAND_COLORS["threeDark"], 'green']
...
### Case 1: Amazon's Last-Mile Delivery (ACO)
...
### Case 2: Hospital Nurse Scheduling (GA + SA)
...
1. **Start Simple**: Greedy baseline first (5 min)
2. **Quick Improvement**: Local search (10 min)  
3. **Smart Enhancement**: Choose ONE metaheuristic (20 min)
4. **Fine-tune**: Adjust parameters (10 min)
5. **Document**: Explain choices (5 min)
...
levels = [
    ...
    ('Metaheuristics', 3, 2, BRAND_COLORS["threeDark"]),
    ('Exact Methods', 4, 1, 'gold')
]
