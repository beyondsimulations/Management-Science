...
Welcome back to Bean Counter! As CEO, you're facing a critical decision: which new coffee product should we add to our menu?
...
- **Energy Smoothie**: Highest profit (€4.5) but slowest to make (270s = 4.5 minutes!)
- **Classic Espresso**: Fastest (60s = 1 minute) but lowest profit (€1.8)
...
fig = plt.gcf()
assert fig.number > 0, "Create a figure with plt.figure()"
ax = plt.gca()
assert len(ax.collections) > 0, "Add points with plt.scatter()"
assert ax.get_xlabel() != '', "Set xlabel for profit margin"
assert ax.get_ylabel() != '', "Set ylabel for prep time"
print("Perfect! The trade-off is now visible!")
print("Lower-right corner = ideal (high profit, fast prep)")
print("But no product is in that corner - we must choose!")
```
...
assert 'Turmeric Latte' in dominated, "Turmeric Latte should be dominated (lower profit, slower than alternatives)"
...
print(products_norm[['Product', 'Profit_Norm', 'Prep_Norm']].head(3).to_string(index=False))
```
...
if len(infeasible) > 0:
    plt.scatter(infeasible['Profit_Margin'], infeasible['Prep_Time'],
               s=100, alpha=0.3, color='red', marker='x', label='Infeasible')
...
if len(feasible_dominated) > 0:
    plt.scatter(feasible_dominated['Profit_Margin'], feasible_dominated['Prep_Time'],
               s=100, alpha=0.4, color='gray', label='Feasible (dominated)')

# Plot constrained Pareto frontier
plt.scatter(constrained_pareto['Profit_Margin'], constrained_pareto['Prep_Time'],
           s=250, alpha=0.9, color='green', edgecolors='black', linewidth=2,
...
::: {.callout-tip}
## Competition Preview
In the fleet optimization competition, you'll:
1. Have 2 objectives (minimize cost, maximize service score)
2. Have 2 hard constraints (CO2 ≤ 111 g/km, 60-120 vehicles)
3. Generate solutions using heuristics
4. Filter for feasibility
5. Find Pareto frontier
6. Create exactly this type of visualization!
:::
