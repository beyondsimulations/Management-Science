...
distance_km = 100  # From Google Maps API
weight_tons = 2.5  # Your shipment
emission_factor = 0.162  # kg CO₂/ton-km (EU truck)
...
- Costs: ±10% (fuel price variation)
- Emissions: ±15% (traffic, weather effects)
...
ax.scatter(sol_df['Cost'], sol_df['Emissions'], 
          alpha=0.3, s=100, color=BRAND_COLORS["darker"], label='Feasible allocations')

# Plot Pareto optimal
ax.scatter(sol_df[pareto_mask]['Cost'], sol_df[pareto_mask]['Emissions'],
          alpha=0.8, s=150, color=PLOT_COLORS[1], label='Pareto optimal', edgecolors=PLOT_COLORS[1], linewidth=2)
...
ax.scatter(sol_df.loc[min_cost_idx, 'Cost'], sol_df.loc[min_cost_idx, 'Emissions'],
          s=300, color=PLOT_COLORS[2], marker='*', label='Min Cost', edgecolors=PLOT_COLORS[2], linewidth=2)
ax.scatter(sol_df.loc[min_emissions_idx, 'Cost'], sol_df.loc[min_emissions_idx, 'Emissions'],
          s=300, color=PLOT_COLORS[0], marker='*', label='Min Emissions', edgecolors=PLOT_COLORS[0], linewidth=2)
...
## Quick Check 2: Which Method to Use? 🤔
...
B) ε-Constraint (handles complexity)  
C) Goal Programming (have targets)

[Think before revealing...]{.fragment}

[**Answer: B) ε-Constraint!**]{.fragment .highlight}
- [Integer variables → Non-convex likely]{.fragment}
- [ε-Constraint finds ALL solutions]{.fragment}
...
## Start Here: Extreme Points Analysis
...
ax1.set_ylabel('Emissions (kg CO₂)', fontsize=12)
...
4. **Stakeholder Communication**: "Best possible cost is €50k, best emissions is 40kg"
:::
...
Different weights → Different "optimal" solutions!

## Method 2: ε-Constraint Method (Epsilon-Constraint)
...
**Intuition:** "Give me the cheapest option that emits less than 100 tons CO₂"
...
::: {.callout-tip}
## Why ε-Constraint Wins
✅ Finds ALL Pareto solutions (including non-convex)  
✅ Works with integer problems  
✅ Easy to understand: "best X given limit on Y"
:::

## Step-by-Step: ε-Constraint Algorithm

**Follow this recipe to implement ε-constraint:**
...
steps = [
    ...
    {'text': 'Step 3: Create ε Grid\nε = [50, 60, 70, ..., 200]', 
     'pos': (0.8, 0.85), 'color': PLOT_COLORS[2]},
    {'text': 'Step 4: For Each ε:\nMin Cost\ns.t. Emissions ≤ ε', 
...
ax.set_xlim(0, 1)
ax.set_ylim(0, 1)
ax.set_title('ε-Constraint Method: Step-by-Step Implementation', 
            fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
```
...
def epsilon_constraint_method(cost_func, emissions_func, constraints, epsilon_grid):
    """
    Complete ε-constraint implementation
...
## Visualizing the Difference: WSM vs ε-Constraint
...
ax1.set_ylabel('Emissions (kg CO₂)', fontsize=11)
...
ax2.plot(x, y_frontier, 'g-', linewidth=3, 
         label='ε-Constraint finds ALL', alpha=0.7)

ax2.set_xlabel('Cost (€)', fontsize=11)
ax2.set_ylabel('Emissions (kg CO₂)', fontsize=11)
ax2.set_title('ε-Constraint Method Success', fontsize=12, fontweight='bold')
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.suptitle('Why ε-Constraint > Weighted Sum for Non-Convex Problems', 
...
# Add annotations
ax.annotate('Small cost increase →\nLarge emission reduction', 
...
ax.annotate('Large cost increase →\nSmall emission reduction', 
...
```
**✓ Fix:** Always normalize to [0,1] first!

**❌ Mistake 2: Using Weighted Sum for Integer Problems**
- Weighted sum misses solutions in non-convex regions
- Integer problems often create non-convex frontiers

**✓ Fix:** Use ε-constraint for discrete variables

**❌ Mistake 3: Assuming Equal Weights = Equal Importance**
```python
# WRONG - Ignores scale differences
w1 = 0.5  # for cost ranging 0-1000
w2 = 0.5  # for emissions ranging 0-10
```
**✓ Fix:** Normalize first, THEN apply weights

**❌ Mistake 4: Optimizing Too Many Objectives**
- 2-3 objectives: Manageable
- 4+ objectives: Exponentially harder

**✓ Fix:** Combine related objectives or use constraints

**❌ Mistake 5: Not Checking Solution Feasibility**
- Pareto optimal ≠ Feasible for your constraints

**✓ Fix:** Always verify constraints after optimization
:::
...
decisions = {
    ...
    'a1': {'text': 'Use ε-Constraint\n✓ Handles non-convex\n✓ Works with integers', 'pos': (0.15, 0.3), 'color': PLOT_COLORS[1]},
    'a2': {'text': 'Try Weighted Sum\nThen ε-Constraint\nif gaps found', 'pos': (0.45, 0.3), 'color': PLOT_COLORS[1]},
    'a3': {'text': 'Use Goal Programming\n✓ Satisficing approach\n✓ Handles targets', 'pos': (0.7, 0.3), 'color': PLOT_COLORS[0]},
    'a4': {'text': 'Use Weighted Sum\n✓ Simple & fast\n✓ Good for continuous', 'pos': (0.9, 0.3), 'color': PLOT_COLORS[1]},
}
...
reference_text = "Quick Rules: Integer problems → ε-constraint | Specific targets → Goal Programming | Simple continuous → Try Weighted Sum first"
...
for idx, (w_cost, w_emissions, title) in enumerate(weight_scenarios):
    ...
    ax.set_ylabel('Emissions (kg CO₂)', fontsize=10)
...
ax.set_xlabel('Total Fleet Cost (k€)', fontsize=12)
ax.set_ylabel('Annual Emissions (tons CO2)', fontsize=12)
ax.set_title('Different Priorities → Different Optimal Fleets', fontsize=14)
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
for name, values in strategies.items():
    ...
    ax.annotate(f"{values['emissions']}g CO₂", 
...
$$\text{Cost Index} = \frac{\text{Cost of Time (€/min)}}{\text{Cost of Fuel (€/kg)}}$$
...
**Real Example:** If flight delayed → High time cost → High CI → Fly faster!
...
ax.set_xlabel('Vehicle Cost (€1000s)', fontsize=12)
...
1. **Forgetting to normalize** - Can't compare € to g/km directly
