...
def visualize_fleet(fleet_composition: Dict[str, int], 
    ...
    if composition_data:
        ...
        axes[0, 0].pie(quantities, labels=types, autopct='%1.0f%%', colors=colors)
    ...
    axes[0, 1].bar(cost_data.keys(), cost_data.values(), color=['#FF6B6B', '#4ECDC4'])
    ...
    axes[1, 0].plot(angles, values_plot, 'o-', linewidth=2, color='#FF6B6B')
    axes[1, 0].fill(angles, values_plot, alpha=0.25, color='#FF6B6B')
    ...
    if vehicle_types:
        ...
        x = range(len(cap_df))
        axes[1, 1].bar(x, cap_df['Capacity'], color='#4ECDC4')
...
axes[0].scatter(alt_df['purchase_cost']/1000, alt_df['service_score'], 
               alpha=0.3, s=50, label='All alternatives')
...
axes[0].scatter(my_metrics['purchase_cost']/1000, my_metrics['service_score'], 
               s=200, color='red', marker='*', label='My solution', zorder=10)
...
if composition_data:
    ...
    wedges, texts, autotexts = ax1.pie(quantities, labels=types, autopct='%1.0f%%', 
                                        colors=colors, textprops={'size': 9})
...
width = 0.35
ax3.bar(x - width/2, my_scores, width, label='Our Fleet', color='#FF6B6B')
ax3.bar(x + width/2, avg_scores, width, label='Average', color='#4ECDC4')
...
ax4.barh(zones['Zone'], zone_capability, color='#95E77E')
...
ax5.bar(annual_costs.keys(), annual_costs.values(), color=['#FF6B6B', '#4ECDC4', '#FFE66D'])
