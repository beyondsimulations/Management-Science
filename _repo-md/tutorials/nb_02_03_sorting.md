...
subtitle: Management Science - Operations Director at Bean Counter!
code-links:
  - text: Python
    href: nb_02_03_sorting.py
    icon: hand-thumbs-up
---
...
> **Empty Lists**
>
> Be careful! Calling `min()` or `max()` on an empty list causes an error.
> Always check if a list has items before using these functions:
>
> ``` python
...
Sort Bean Counter stores by efficiency (customers served per staff member).
Create two rankings:
...
It's 7 AM at Bean Counter Plaza, and you have multiple drink orders to schedule.
Apply both SPT and EDD rules to determine the order sequence.
