...
# [When Monte Carlo?]{.flow} {.title}

## When to Use Monte Carlo

[Question]{.question}: For our simple startup examples so far, do we really NEED Monte Carlo?
...
[So when is Monte Carlo truly necessary?]{.highlight}

## Necessary vs. Convenient

[You can use math (analytical solutions):]{.highlight}
...
[You NEED Monte Carlo when:]{.highlight}
...
[Where Monte Carlo is ESSENTIAL, not just convenient:]{.highlight}
...
setup_clean_style()
fig, ax = plt.subplots()
ax.hist(payoffs, bins=50, alpha=0.7, color=BRAND_COLORS["twoDark"],
        edgecolor='black', linewidth=0.5)
ax.axvline(payoffs.mean(), color=BRAND_COLORS["threeDark"], linestyle='--',
           linewidth=2, label=f'Mean: ${payoffs.mean():.2f}')
ax.axvline(np.percentile(payoffs, 95), color=BRAND_COLORS["oneDark"],
           linestyle=':', linewidth=2, label=f'95th percentile: ${np.percentile(payoffs, 95):.2f}')
ax.set_xlabel('Option Payoff ($)', fontsize=12)
ax.set_ylabel('Frequency', fontsize=12)
ax.set_title('Distribution of Option Payoffs', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(True, alpha=0.2)
plt.tight_layout()
plt.show()
...
#| eval: true
#| echo: false
# Visualize supply chain revenue distribution
setup_clean_style()
fig, ax = plt.subplots()
ax.hist(revenues, bins=30, alpha=0.7, color=BRAND_COLORS["oneDark"],
        edgecolor='black', linewidth=0.5)
ax.axvline(revenues.mean(), color=BRAND_COLORS["threeDark"], linestyle='--',
           linewidth=2, label=f'Mean: ${revenues.mean():.2f}')
ax.axvline(demand * price, color=BRAND_COLORS["twoDark"], linestyle=':',
           linewidth=2, label=f'Target (meet demand): ${demand * price:.2f}')
ax.set_xlabel('Revenue ($)', fontsize=12)
ax.set_ylabel('Frequency', fontsize=12)
ax.set_title('Distribution of Supply Chain Revenues', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(True, alpha=0.2)
plt.tight_layout()
plt.show()
...
#| eval: true
#| echo: false
# Visualize project completion time distribution
setup_clean_style()
fig, ax = plt.subplots()
ax.hist(project_times, bins=50, alpha=0.7, color=BRAND_COLORS["twoLight"],
        edgecolor='black', linewidth=0.5)
ax.axvline(project_times.mean(), color=BRAND_COLORS["threeDark"], linestyle='--',
           linewidth=2, label=f'Mean: {project_times.mean():.1f} days')
ax.axvline(np.percentile(project_times, 90), color=BRAND_COLORS["oneDark"],
           linestyle=':', linewidth=2, label=f'90th %ile: {np.percentile(project_times, 90):.1f} days')
ax.axvline(100, color='red', linestyle='-',
           linewidth=2, alpha=0.5, label='Target: 100 days')
ax.set_xlabel('Project Duration (days)', fontsize=12)
ax.set_ylabel('Frequency', fontsize=12)
ax.set_title('Distribution of Project Completion Times', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(True, alpha=0.2)
plt.tight_layout()
plt.show()
...
[Question]{.question}: If we can calculate variance analytically, why simulate?
...
[Even when you CAN use Monte Carlo, sometimes you shouldn't:]{.highlight}
