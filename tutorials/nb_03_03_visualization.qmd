...
# 2. Create a bar chart
plt.figure(figsize=(10, 6))
plt.bar(products_sorted['product'], products_sorted['units_sold'], color='#363D45')
...
#| eval: false
plt.figure(figsize=(10, 6))
plt.plot(months, customers, marker='o', linewidth=2, markersize=8, color='#EBADAD')
...
plt.figure(figsize=(10, 6))
plt.hist(efficiency_scores, bins=10, color='#A7C7C6', alpha=0.7, edgecolor='black')
...
for i in range(12):
    # Apply 2% growth and random variation
    monthly_revenue = current_revenue * np.random.uniform(0.75, 1.25)
    revenue_projection.append(monthly_revenue)
    # Update current revenue with 2% growth for next month
    current_revenue = current_revenue * 1.02
...
# Monthly revenue bars (first 6 months as "Actual", rest as "Projected")
axes[0].bar(projections_df['month'][:6], projections_df['revenue'][:6],
           color='#2E7D32', alpha=0.7, label='H1 Actual')
axes[0].bar(projections_df['month'][6:], projections_df['revenue'][6:],
           color='#FFA726', alpha=0.7, label='H2 Projected')
...
print(f"\nYear-end projection: ${projections_df['cumulative'].iloc[-1]:.2f}M")
