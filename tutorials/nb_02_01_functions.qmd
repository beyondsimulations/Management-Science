...
Congratulations! You've successfully started as assistant manager!

You've successfully standardized operations across all Bean Counter locations!

**You've learned:**

- reusable functions to standardize pricing across all stores
- smart business logic for discounts and quality control
- modular systems where functions work together seamlessly
- returning multiple values for comprehensive reporting


Remember the chaos we started with? Different prices at each store, inconsistent discounts, and frustrated customers? Functions have solved all of that! Now:

- Every customer gets consistent pricing
- Discounts are applied fairly and automatically
- Quality standards are maintained across all locations
- New baristas can be trained quickly using your standardized functions

**Key Takeaways:**

1. **Functions create consistency** - Like recipe cards, they ensure everyone does things the same way
2. **Functions are composable** - Simple functions can be combined to handle complex operations
3. **Tuples enable rich returns** - Functions can return multiple pieces of information at once
4. **Business logic belongs in functions** - This makes policies easy to update and maintain

The CEO is impressed with your work! In the next tutorial, you'll be promoted to **Regional Manager**, where you'll use **dictionaries** to manage data across all Bean Counter locations. You'll track inventory, analyze performance metrics, and optimize operations across the entire chain!
