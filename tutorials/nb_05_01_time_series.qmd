...
Use matplotlib to understand the data in depth
...
## Exercise 5.1 - Bean Counter's Seasonality

Before applying Holt-Winters, let's confirm Bean Counter has seasonal patterns.
...
monthly_avg =   # Group by month and calculate mean sales

# Find peak and low seasons
peak_month =   # Month with highest average sales (1-12)
low_month =    # Month with lowest average sales (1-12)

# Calculate seasonal strength
seasonal_range =   # Difference between peak and low month averages
overall_mean = df['sales'].mean()
seasonal_strength =   # seasonal_range / overall_mean (as percentage)
```
...
overall_mean = df['sales'].mean()
seasonal_strength = seasonal_range / overall_mean
```
:::
...
assert 5 <= peak_month <= 8, f"Peak should be summer, got month {peak_month}"
assert low_month in [1, 2, 12], f"Low should be winter, got month {low_month}"
assert seasonal_strength > 0.15, "Should have noticeable seasonality (>15%)"
print(f"✓ Seasonality detected!")
print(f"  Peak: {month_names[peak_month]} ({monthly_avg[peak_month]:.0f} drinks)")
print(f"  Low: {month_names[low_month]} ({monthly_avg[low_month]:.0f} drinks)")
print(f"  Seasonal strength: {seasonal_strength:.1%}")
```

## Exercise 5.2 - Apply Holt-Winters
