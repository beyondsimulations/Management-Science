---
title: "Tutorial 2.3 - Sorting & Finding Best Options"
...
**Your Mission:** Master **sorting and optimization** - Python's tools for finding the best (or worst) options among many choices. You'll learn to rank, prioritize, and optimize like a true operations expert.
...
```{python}
#| eval: false
fastest_first = sorted(wait_times)
slowest_first = sorted(wait_times, reverse=True)
...
Create an alphabetically sorted list of store names for the company directory.
...
```{python}
#| eval: false
alphabetical = sorted(store_names)
...
# YOUR CODE BELOW
# Calculate efficiency for each store first
...
# Sort by efficiency (most efficient first)
most_efficient = sorted(store_efficiency, key=lambda x: x["efficiency"], reverse=True)

# Sort by efficiency (least efficient first)
least_efficient = sorted(store_efficiency, key=lambda x: x["efficiency"])
...
3. The store with the best efficiency ratio (usage/inventory)
...
# Calculate days_remaining for each store, then find most critical
for store in supply_data:
    store["days_remaining"] = store["inventory_kg"] / store["daily_usage"]

most_critical =

print(f"Most urgent (lowest inventory): {most_urgent['store']}")
print(f"Highest demand: {highest_demand['store']}")
print(f"Most critical (fewest days left): {most_critical['store']}")
```
...
#| eval: false
# Find store with lowest inventory
most_urgent = min(supply_data, key=lambda x: x["inventory_kg"])

# Find store with highest daily usage
highest_demand = max(supply_data, key=lambda x: x["daily_usage"])

# Calculate days_remaining for each store, then find most critical
for store in supply_data:
    store["days_remaining"] = store["inventory_kg"] / store["daily_usage"]

most_critical = min(supply_data, key=lambda x: x["days_remaining"])
...
assert most_critical["store"] == "Downtown", "Downtown has fewest days remaining"
...
spt_sorted =
spt_sequence = [order["id"] for order in spt_sorted]

# Apply EDD rule (sort by promised_time)
edd_sorted =
edd_sequence = [order["id"] for order in edd_sorted]
...
#| eval: false
# Apply SPT rule (sort by prep_time)
spt_sorted = sorted(rush_orders, key=lambda x: x["prep_time"])
spt_sequence = [order["id"] for order in spt_sorted]

# Apply EDD rule (sort by promised_time)
edd_sorted = sorted(rush_orders, key=lambda x: x["promised_time"])
edd_sequence = [order["id"] for order in edd_sorted]
...
**What's Next:** The CEO is preparing you for the ultimate challenge! In the next tutorial, you'll integrate everything you've learned - functions, dictionaries, and sorting - in a comprehensive recap that solidifies your mastery of Python for business operations!
