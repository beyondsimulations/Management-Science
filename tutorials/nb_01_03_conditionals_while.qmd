...
Combining loops with conditionals lets us filter and process data intelligently.
...
::: callout-tip
Instead of just using `range`, we can use `enumerate` to get both index and value in a loop at once. The `enumerate` function returns the index (here i) and value (here sale) of each element in the list.
:::
