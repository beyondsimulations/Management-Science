...
for i in range(n_orders):
    ...
    if i > 0:
        current_arrival += np.random.exponential(3.5)

    processing = np.random.choice([3, 5, 7, 10], p=[0.35, 0.35, 0.20, 0.10])

    # Due times are relative to arrival (customers want drinks soon after ordering)
    order_urgency = np.random.choice(['rush', 'normal', 'relaxed'], p=[0.25, 0.50, 0.25])
    if order_urgency == 'rush':
        due = current_arrival + processing + np.random.randint(5, 15)
    elif order_urgency == 'normal':
        due = current_arrival + processing + np.random.randint(15, 30)
    else:
        due = current_arrival + processing + np.random.randint(30, 50)
...
def schedule_spt_dynamic(orders):
    ...
    while remaining:
        ...
        next_order = # YOUR CODE: min(available, key=...)
...
assert df_friday_dynamic_spt['completion'].max() < df_friday_static_spt['completion'].max(), \
    "Dynamic SPT should beat static SPT on makespan"
...
print(f" • Static makespan: {df_friday_static_spt['completion'].max():.1f} minutes")
print(f" • Dynamic makespan: {df_friday_dynamic_spt['completion'].max():.1f} minutes")
print(f" • Improvement: {df_friday_static_spt['completion'].max() - df_friday_dynamic_spt['completion'].max():.1f} minutes faster")
print(f" • Static idle time: {static_idle:.1f} minutes")
...
def schedule_edd_dynamic(orders):
    ...
    while remaining:
        ...
        # YOUR CODE: Choose earliest due date (HINT: change key from SPT)
