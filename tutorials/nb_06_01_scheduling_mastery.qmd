...
def schedule_fifo(orders):
    ...
    # The lambda function tells sorted() to sort by the 'arrival' value in each order
...
Now implement SPT to always process the shortest available job next.
...
print(f"\nCEO Decision: Deploy {best_rule} to all global locations!")
```

## CEO's Global Rollout Decision Dashboard

This dashboard shows what will happen when you deploy your chosen scheduling rule to all 1,247 Bean Counter locations worldwide.

```{python}
#| eval: true
# Create executive dashboard for CEO decision
# It should include the average wait time, late orders and total tardiness
# YOUR CODE BELOW
...
plt.suptitle('Bean Counter NYC Flagship - CEO Global Rollout Dashboard', fontsize=16, fontweight='bold')
plt.show()
```
:::
...
**Great work, CEO!** You've mastered the fundamental scheduling algorithms that power operations worldwide. More importantly, you've tested them on **real flagship store data** and can now confidently make a strategic decision.

## Your Global Impact

By solving the scheduling problem at your NYC flagship store, you've created a system that will:

::: {.callout-important}
## Bean Counter Global Rollout Impact
- **Thousand stores** worldwide will implement your chosen algorithm
- **Millions of customers** annually will benefit from optimized service
- **Operational costs** reduced through better resource utilization
- **Customer satisfaction** improved through predictable service times
- **Competitive advantage** through data-driven operations management
:::
...
Your greedy algorithms work well, but can we do even better? In the next notebook, you'll tackle the **Bike Factory Competition** where you'll apply these scheduling techniques to a real two-stage manufacturing problem.
