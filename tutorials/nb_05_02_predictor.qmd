...
Forecast December 2024 sales (weeks 1-4) for three key products using 2 years of historical weekly data.
...
# Generate 2 years of realistic weekly sales data
weeks = pd.date_range(start='2023-01-03', end='2024-12-01', freq='W-MON')
...
def generate_product_sales(trend, seasonality_type, volatility, base=100):
    ...
    # Base trend
    if trend == 'growing':
        trend_component = np.linspace(base, base * 1.5, n_weeks)
    elif trend == 'stable':
        trend_component = np.ones(n_weeks) * base
    else:  # declining
        trend_component = np.linspace(base, base * 0.7, n_weeks)
    ...
    if seasonality_type == 'christmas':
        ...
        seasonal = 1 + 0.3 * np.cos(2 * np.pi * np.arange(n_weeks) / 52)
    else:  # moderate
...
techpod_sales = generate_product_sales('growing', 'moderate', 0.15, base=250)
fitband_sales = generate_product_sales('stable', 'newyear', 0.25, base=180)
cozythrow_sales = generate_product_sales('stable', 'christmas', 0.20, base=120)
...
for idx, (product, color) in enumerate(zip(products, colors)):
    ...
    axes[idx].set_title(f'{product.replace("_", " ").title()} - 2 Year History')
...
```{python}
#| eval: true
# Define the December 2024 weeks we need to forecast
december_weeks = pd.date_range(start='2024-12-02', end='2024-12-23', freq='W-MON')
print("December 2024 weeks to forecast:")
