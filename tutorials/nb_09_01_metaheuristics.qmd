...
Work through each section in order. Write code where marked "YOUR CODE BELOW" and verify with the provided assertions. You'll implement greedy, local search, simulated annealing, and genetic algorithms, the **exact techniques** you'll need for the Restaurant Staffing Competition!
:::
...
We'll use this function to determine which cafés are assigned to which centers for visualization:
...
def greedy_center_selection(cafe_locs: List[Tuple[float, float]],
    ...
    for step in range(n_select):
        ...
        for candidate in remaining:
            ...
            # Calculate cost with balanced allocation
            cost = # YOUR CODE
...
for center_idx, cafe_indices in greedy_assignments.items():
    ...
    for cafe_idx in cafe_indices:
        cafe_loc = cafe_locations[cafe_idx]
        plt.plot([center_loc[1], cafe_loc[1]], [center_loc[0], cafe_loc[0]], 
                'lightgreen', alpha=0.3, linewidth=1.5, zorder=1)
...
plt.scatter(selected_lons, selected_lats, s=150, alpha=1.0, color='green',
           marker='*', label='Greedy Selected (50)', edgecolors='darkgreen', 
           linewidth=1, zorder=3)

plt.xlabel('Longitude (°)', fontsize=12)
plt.ylabel('Latitude (°)', fontsize=12)
plt.title(f'Greedy Solution: €{greedy_cost:,.0f}/year ({greedy_improvement_pct:.1f}% improvement)\n(Green lines show café-to-center assignments)',
         fontsize=14, fontweight='bold')
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
**Can we do better?** Yes! That's where metaheuristics come in.
:::

# Section 3: Local Search - Building on Notebook 7.1

Remember **2-opt** from the routing notebook? Let's apply the same concept here!

::: {.callout-tip}
## Local Search Review
**Core idea**: Start with a solution, repeatedly make small changes that improve it.

**In Routing**: We swapped edges in a route
**Here**: We'll swap centers in/out
...
## Bonus: 2-Opt Neighborhood (Advanced)

::: {.callout-tip}
## Why Try 2-Opt?

**2-opt** swaps TWO centers simultaneously, creating a richer neighborhood:

- **1-opt**: 50 × 150 = 7,500 possible neighbors
- **2-opt**: C(50,2) × C(150,2) ≈ 13.7 million possible neighbors!

This larger neighborhood can help escape local optima that trap 1-opt. Try both in your SA implementation to see which finds better solutions!
:::

```{python}
#| eval: true
def generate_2opt_neighbor(current_selection: List[int],
                          n_centers: int = 200) -> List[int]:
    """Generate a neighbor by swapping TWO selected centers with TWO unselected (2-opt)."""
    # Get unselected centers
    unselected = [i for i in range(n_centers) if i not in current_selection]
    
    # Pick 2 random selected centers to remove
    if len(current_selection) >= 2 and len(unselected) >= 2:
        remove_indices = random.sample(range(len(current_selection)), 2)
        add_centers = random.sample(unselected, 2)
    else:
        # Fallback to 1-opt if not enough centers
        return generate_swap_neighbor(current_selection, n_centers)
    
    # Create new selection
    new_selection = current_selection.copy()
    
    # Remove the selected centers (in reverse order to maintain indices)
    for idx in sorted(remove_indices, reverse=True):
        new_selection.pop(idx)
    
    # Add the new centers
    new_selection.extend(add_centers)
    
    return new_selection
```

::: {.callout-note}
You now have TWO neighborhood functions:

- `generate_swap_neighbor` (1-opt): Simple, fast, good for local refinement
- `generate_2opt_neighbor` (2-opt): Richer, better escape from local optima

**Challenge**: Try both in your Local Search and SA implementations! 

- Replace `generate_swap_neighbor` with `generate_2opt_neighbor` in the algorithms
- Which finds better solutions? 
- Is 2-opt worth the extra complexity?
- Does SA benefit more from 2-opt than Local Search?
:::
...
def local_search(initial_solution: List[int],
    ...
    for iteration in range(max_iterations):
        ...
        for _ in range(neighbors_per_iteration):
            ...
            # Evaluate neighbor
            neighbor_cost = # YOUR CODE

            # If better, keep it
            if neighbor_cost < current_cost:
                # YOUR CODE: update current, current_cost, improvements
                improved = True
                break
...
**This is exactly why we need metaheuristics!**
:::
...
**The key insight**: Sometimes accept **worse** solutions to escape local optima!

**Temperature analogy**:
- **High temperature** (early): Accept bad moves frequently (explore widely)
- **Low temperature** (late): Accept bad moves rarely (refine solution)
...
if new_cost < current_cost:
    ...
    accept = random.random() < probability  # Sometimes accept worse
```

**As temperature decreases**: Acceptance probability → 0, SA behaves like local search
:::
...
def accept_move(current_cost: float, new_cost: float, temperature: float) -> bool:
    ...
    if new_cost < current_cost:
        ...
        delta = # YOUR CODE
        probability = # YOUR CODE: exp(-delta / temperature)
...
Now put it all together! We'll provide the cooling schedule, you implement the main loop.
...
This gives us ~1,500 iterations of exploration before becoming pure greedy.
:::
...
def simulated_annealing(initial_solution: List[int],
    ...
    while temperature > min_temp:
        ...
        # 3. Decide whether to accept using accept_move function
        if accept_move(current_cost, neighbor_cost, temperature):
            # YOUR CODE: update current and current_cost
            pass

            # Track best ever found
            if current_cost < best_cost:
                # YOUR CODE: update best and best_cost
                pass
...
for center_idx, cafe_indices in sa_assignments.items():
    ...
    for cafe_idx in cafe_indices:
        cafe_loc = cafe_locations[cafe_idx]
        plt.plot([center_loc[1], cafe_loc[1]], [center_loc[0], cafe_loc[0]], 
                'gold', alpha=0.3, linewidth=1.5, zorder=1)
...
print(f"\n💡 Notice how SA found a different configuration than Local Search!")
...
- **Early exploration** (high T): SA accepts ~60% of worse moves, exploring far from greedy solution
...
results = pd.DataFrame({
    ...
    'Computation Time': ['0 sec', '~30 sec', '~45 sec', '~60 sec']
})
...
**Congratulations, CEO!** You've mastered metaheuristics and saved Bean Counter millions!

## Key Takeaways

1. **Problem Scale Matters**
   - 1,000 cafés × 200 centers = 10^74 possible networks
   - Impossible to try them all, must use smart search

2. **Balanced Allocation Simplifies Everything**
   - Each center serves exactly 20 cafés
   - Greedy assignment ensures fairness
   - No feasibility issues to worry about!

3. **Algorithm Progression**
   - **Greedy**: Fast, shortsighted
   - **Local Search**: Gets stuck
   - **Simulated Annealing**: Escapes traps

4. **The Power of Accepting Worse Moves**
   - Local search: Never accept worse → stuck
   - SA: Temporarily accept worse → escape
   - Temperature controls exploration vs. exploitation

5. **Business Impact**
   - Millions in annual savings with SA
   - 10-year contract worth tens of millions
   - ROI: 1000x+ on optimization effort
