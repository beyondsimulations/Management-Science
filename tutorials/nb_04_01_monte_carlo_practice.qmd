...
This notebook is structured to progressively build your skills:
1. **Section 1:** Model single uncertain variables
2. **Section 2:** Combine uncertainties in one business
3. **Section 3:** Analyze risk metrics
4. **Section 4:** Compare portfolio combinations (key for competition!)
5. **Section 5:** Make strategic recommendations

Each exercise includes assertions to check your work. Green checks (✓) mean you're on track!
...
print("✓ Purchase simulation correct!")
```
...
Using your simulated data, calculate key probabilities.
...
assert 200 < mean_profit < 300, f"Mean profit should be ~250, got {mean_profit:.2f}"
assert 0.02 < prob_loss < 0.08, f"Probability of loss should be ~5%, got {prob_loss:.1%}"
...
Create a histogram of profit distribution with key markers.

```{python}
#| eval: false
# YOUR CODE BELOW
# Create a histogram of the profit distribution
plt.figure(figsize=(10, 6))

# Create histogram (use 50 bins)


# Add vertical lines for mean (green) and break-even (red)


# Add labels and title
plt.xlabel('Daily Profit (€)')
plt.ylabel('Frequency')
plt.title('Coffee Shop Daily Profit Distribution (10,000 simulations)')
plt.legend()
...
# Don't modify below
assert 0.85 < prob_target_range < 0.95, f"Should be ~90% in range, got {prob_target_range:.1%}"
...
# Assume risk-free rate is €50 (alternative investment)
risk_free = 50
sharpe_ratio =

# Don't modify below
assert 110 < volatility < 130, f"Volatility should be ~120, got {volatility:.2f}"
assert 0.55 < prob_good_day < 0.75, f"Probability should be ~65%, got {prob_good_day:.1%}"
assert 50 < expected_shortfall < 100, f"ES should be ~75, got {expected_shortfall:.2f}"
assert 1.5 < sharpe_ratio < 2.5, f"Sharpe ratio should be ~2.0, got {sharpe_ratio:.2f}"
...
risk_free = 50
sharpe_ratio = (profits_array.mean() - risk_free) / volatility
...
# Section 4 - Portfolio Analysis (Competition Training!)

Now we'll analyze portfolios of coffee shops - directly applicable to the TechVenture competition!
...
assert 'Downtown + Campus' in best_mean_portfolio or 'Campus + Downtown' in best_mean_portfolio, \
...
## Exercise 4.2 - Risk-Adjusted Selection

The best mean return isn't always the best choice. Find the best risk-adjusted portfolio.

```{python}
#| eval: false
# YOUR CODE BELOW
# Calculate Sharpe ratio for each portfolio (assuming risk-free = €100)
risk_free_rate = 100
...
df_portfolios['sharpe'] = sharpe_ratios

# Find best Sharpe ratio portfolio
df_portfolios_sharpe = df_portfolios.sort_values('sharpe', ascending=False)
best_sharpe_portfolio =
best_sharpe_value =

# Also find the safest portfolio (lowest probability of loss)
safest_portfolio =
safest_prob_loss =

# Don't modify below
assert best_sharpe_value > 2.0, f"Best Sharpe should be >2.0, got {best_sharpe_value:.2f}"
assert safest_prob_loss < 0.01, f"Safest should have <1% loss probability, got {safest_prob_loss:.3f}"
print("✓ Risk-adjusted analysis correct!")
print(f"\nBest Sharpe Ratio: {best_sharpe_portfolio} (Sharpe: {best_sharpe_value:.2f})")
print(f"Safest Portfolio: {safest_portfolio} (P(Loss): {safest_prob_loss:.1%})")
```
...
sharpe_ratios = []
for _, row in df_portfolios.iterrows():
    sharpe = (row['mean'] - risk_free_rate) / row['std']
    sharpe_ratios.append(sharpe)

df_portfolios['sharpe'] = sharpe_ratios

df_portfolios_sharpe = df_portfolios.sort_values('sharpe', ascending=False)
best_sharpe_portfolio = df_portfolios_sharpe.iloc[0]['pair']
best_sharpe_value = df_portfolios_sharpe.iloc[0]['sharpe']

df_portfolios_safe = df_portfolios.sort_values('prob_loss')
safest_portfolio = df_portfolios_safe.iloc[0]['pair']
safest_prob_loss = df_portfolios_safe.iloc[0]['prob_loss']
...
Now let's visualize and make a final recommendation - just like in the competition!
...
# Which portfolio has the best risk-adjusted return (Sharpe)?
best_balanced_portfolio =

# Your final recommendation (choose one)
final_recommendation = # Choose the portfolio you would recommend

# Justification score (0-100) based on multiple criteria
# Higher score = better choice
def score_portfolio(pair_name):
    row = df_portfolios[df_portfolios['pair'] == pair_name].iloc[0]
    score = 0
    score += min(40, row['mean'] / 10)  # Up to 40 points for profit
    score += min(30, (1 - row['prob_loss']) * 30)  # Up to 30 points for low loss probability
    score += min(30, row['sharpe'] * 10)  # Up to 30 points for Sharpe ratio
    return score
...
assert recommendation_score > 60, f"Your choice scores {recommendation_score:.1f}/100 - try a different portfolio"
print("✓ Recommendation accepted!")
print(f"\nYour Recommendation: {final_recommendation}")
print(f"Score: {recommendation_score:.1f}/100")
print("\nJustification: This portfolio balances profit potential with acceptable risk.")
```
...
best_balanced_portfolio = df_portfolios_sharpe.iloc[0]['pair']

# Choose the balanced option
final_recommendation = best_balanced_portfolio

def score_portfolio(pair_name):
    row = df_portfolios[df_portfolios['pair'] == pair_name].iloc[0]
    score = 0
    score += min(40, row['mean'] / 10)
    score += min(30, (1 - row['prob_loss']) * 30)
    score += min(30, row['sharpe'] * 10)
    return score
...
## Creating Your One-Slide Summary

```{python}
#| eval: true
# Generate summary statistics for the recommended portfolio
recommended = df_portfolios[df_portfolios['pair'] == best_balanced_portfolio].iloc[0]

print("=" * 60)
print(" ONE-SLIDE RECOMMENDATION: BEAN COUNTER EXPANSION STRATEGY")
print("=" * 60)
print(f"\n➤ RECOMMENDATION: Open {recommended['pair']}")
print(f"\n➤ KEY METRICS:")
print(f"   • Expected Daily Profit: €{recommended['mean']:.2f}")
print(f"   • Risk (Std Dev): €{recommended['std']:.2f}")
print(f"   • Probability of Loss: {recommended['prob_loss']:.1%}")
print(f"   • Value at Risk (5%): €{recommended['var_5']:.2f}")
print(f"   • Sharpe Ratio: {recommended['sharpe']:.2f}")
print(f"\n➤ JUSTIFICATION:")
print(f"   • Strong risk-adjusted returns (Sharpe > 2.0)")
print(f"   • Minimal downside risk (<1% chance of loss)")
print(f"   • Diversified customer base reduces volatility")
print("=" * 60)
```
