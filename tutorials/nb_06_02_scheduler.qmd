...
**It's Friday 2:00 PM.** The production manager just quit unexpectedly, leaving you with a major problem:

- **16 custom bicycle orders** received this week - all promised for delivery Monday
...
- **Skeleton crew:** Weekend staffing is minimal - only one technician per station
- **Cost pressures:**
  - Overtime costs **€100/hour** for any work after Saturday 8 PM (minute 1800)
...
Schedule all 16 bicycle orders through both workstations to **minimize total cost** (overtime + late penalties).

### The Two-Stage Process

Each bicycle must follow this sequence:

1. **Assembly Station:** Frame assembly, component installation
2. **Painting Station:** Surface prep, painting, curing (can only start AFTER assembly completes)

**Critical Constraint:** A bike cannot begin painting until its assembly is finished!

### Success Metrics

Your schedule will be evaluated on:

1. **Total Cost (70%):** Overtime costs + Late delivery penalties
2. **Strategy Clarity (30%):** Clear explanation of your approach

### Financial Impact

**Overtime Costs:**

- Regular hours: Friday 2 PM - Saturday 8 PM (0 minutes - 1800 minutes)
- Overtime: After Saturday 8 PM (minute 1800+)
- Rate: €100 per hour (€1.67 per minute)

**Late Penalties:**

- Rush orders: €150 per late delivery
- Standard orders: €50 per late delivery
- Custom orders: €100 per late delivery

**Example Cost Calculation:**

- If painting finishes at minute 1860 (60 min overtime): 60 ÷ 60 × €100 = **€100**
- If order B02 (rush) completes after minute 360: **€150 penalty**
- Total cost for this scenario: **€250**
...
print("Custom Cycles - Weekend Rush Orders")
print("=" * 50)
...
print(f"\nTime constraints:")
print(f"  Work starts: Friday 2:00 PM (minute 0)")
print(f"  Regular hours end: Saturday 8:00 PM (minute 1800)")
...
ax.axvline(x=1800, color='red', linestyle='--', label='Overtime starts', linewidth=2)
...
```{python}
#| eval: true
def calculate_schedule_cost(schedule, overtime_threshold=1800, overtime_rate_per_min=100/60):
    """
    Calculate total cost for a given schedule

    Parameters:
    - schedule: list of orders with completion times
    - overtime_threshold: minute when overtime starts (default 1800)
