...
def generate_techmart_sales_history():
    ...
    for category, count in categories.items():
        for i in range(count):
            ...
            if category == 'Smartphones':
                ...
                base_demand = np.random.uniform(90, 320)
    ...
    for _, sku in skus_df.iterrows():
        for week_start in weeks:
            ...
            # Component 1: Annual cycle (52 weeks) - peak in Nov/Dec (Q4), trough in summer (Q3)
            annual_seasonality = 1.0 + sku['seasonal_strength'] * np.cos(2 * np.pi * (week_num - 47) / 52)

            # Component 2: Quarterly cycle (13 weeks) - shorter-term fluctuations
...
# Get top 3 highest base_demand products for legend
top_skus = skus_df.nlargest(3, 'base_demand')['sku_id'].values
...
for sku_id in skus_df['sku_id']:
    ...
    # Only label top 3 SKUs by base demand
    label = f"{sku_id} (base={sku_info['base_demand']:.0f})" if sku_id in top_skus else ''
    plt.plot(sku_history['week_start'], sku_history['units_sold'],
            alpha=0.4, linewidth=0.8, label=label)

plt.xlabel('Date', fontsize=12)
plt.ylabel('Units Sold', fontsize=12)
plt.title('All 30 SKUs Sales History (2023-2025)\nDual Seasonality with Product-Specific Characteristics',
         fontsize=14, fontweight='bold')
