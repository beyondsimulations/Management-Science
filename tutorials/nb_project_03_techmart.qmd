...
*Here's the critical constraint: Once Black Friday starts, we CANNOT refill the Hamburg warehouse. All our warehouse staff are 100% busy picking and packing orders and there's zero time for restocking. Whatever allocation we make beforehand is what we're stuck with for the entire weekend!*
...
**Your Goal:** Decide how many units of each SKU go in Hamburg warehouse to **minimize total delay penalty costs**.
...
This is a classic **knapsack problem**: Which units should we "pack" into the limited Hamburg warehouse capacity?
...
def generate_techmart_sales_history():
    ...
    Realistic patterns:
    - Seasonality (Black Friday spike, summer dip)
    - Product lifecycle (new products, declining products)
    - Different demand levels (bestsellers vs. niche items)
    """

    # Define 30 SKUs across 5 categories
    categories = {
        'Smartphones': 8,
        'Laptops': 6,
        'Headphones': 6,
    ...
    for category, count in categories.items():
        for i in range(count):
            # Base demand level (some products are naturally more popular)
            base_demand = np.random.choice(
                [30, 80, 150, 250],  # Units per week
                p=[0.35, 0.35, 0.20, 0.10]  # Distribution of demand levels
            )

            # Product age (affects lifecycle)
            launch_date = np.random.choice(['2023-01', '2024-01', '2025-01'])
    ...
    for _, sku in skus_df.iterrows():
        sku_launch = datetime.strptime(sku['launch_date'], '%Y-%m')

        for week_start in weeks:
            # Skip if product not yet launched
            if week_start < sku_launch:
                continue
            ...
            # Seasonality factors
            # Black Friday (week 47-48)
            if week_num in [47, 48]:
                demand *= np.random.uniform(2.5, 3.5) 

            # Summer dip (June-August)
            elif month in [6, 7, 8]:
                demand *= np.random.uniform(0.6, 0.8)

            # December holiday season (except BF weeks)
            elif month == 12:
                demand *= np.random.uniform(1.3, 1.7)

            # Product lifecycle
            weeks_since_launch = (week_start - sku_launch).days / 7

            # Growth phase (first 6 months)
            if weeks_since_launch < 26:
                lifecycle_factor = 0.5 + (weeks_since_launch / 26) * 0.5
            # Maturity (6 months - 2 years)
            elif weeks_since_launch < 104:
                lifecycle_factor = 1.0
            # Decline (after 2 years)
            else:
                lifecycle_factor = max(0.3, 1.0 - ((weeks_since_launch - 104) / 52) * 0.3)

            demand *= lifecycle_factor
...
print(f"\n2024 Expected Black Friday total (if similar to 2023-2024): ~{avg_2023_2024:.0f} units")
...
# Plot example SKUs - one high-demand, one low-demand
high_demand_sku = skus_df.nlargest(1, 'base_demand').iloc[0]['sku_id']
low_demand_sku = skus_df.nsmallest(1, 'base_demand').iloc[0]['sku_id']

print("\nHigh-demand SKU example:")
plot_sku_sales_trend(high_demand_sku, sales_df, skus_df)

print("\nLow-demand SKU example:")
plot_sku_sales_trend(low_demand_sku, sales_df, skus_df)
```
...
- **Demand Forecasting:**
  ...
  - Validation: Test your model to assess accuracy
...
- **Inventory Allocation Optimization:**
  ...
  - This is a **knapsack problem**: Rank SKUs and allocate greedily, or use optimization

- **Monte Carlo Simulation:**

  - Generate 10,000 demand scenarios with ±20% variation from your forecast
...
- **Problem understanding** (1 min): Yola's crisis in your own words
- **Your approach** (2 min): Forecasting method + allocation logic
- **Results** (3 min): Forecasts, allocation decision, Monte Carlo analysis
- **Business impact** (1 min): €€€ savings, recommendations
- **Q&A preparation** (1 min): Trade-offs, limitations
...
- Helper functions: `get_sku_sales_history()`, `calculate_allocation_cost()`
