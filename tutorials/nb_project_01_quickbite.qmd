...
**Company Size:** 12 employees, 100+ daily deliveries
...
- **Daily deliveries:** 100 meal orders across Hamburg
...
- **Driver capacity:** Each driver can carry up to 28 meals at once
- **Average order value:** €10

### Cost Structure

- **Driver wages:** €18/hour (including benefits)
- **Fuel costs:** €0.45 per km
- **Late delivery penalty:** €5 per late delivery (customer discount/refund)
- **Cold food complaints:** €10 per complaint (full refund policy)

### Customer Expectations

Each delivery has a **time window** (requested delivery time ± 15 minutes):

- Orders placed for lunch rush (12:00-13:30)
- Orders placed for dinner rush (18:00-20:30)
- Premium customers pay extra for "on-time guarantee"

**Time window violation = Late penalty + High chance of cold food complaint**
...
3. **Respects vehicle capacity** (max 8 meals per driver at once)
...
# Generate 100 delivery locations across Hamburg
# Hamburg spans roughly: lat 53.45-53.65, lon 9.85-10.15
def generate_hamburg_deliveries(n=100):
    deliveries = []

    # Create realistic clusters (residential areas, business districts)
    clusters = [
        {'lat': 53.5511, 'lon': 10.0063, 'n': 25, 'type': 'city_center'},      # Altstadt
        {'lat': 53.5534, 'lon': 9.9700, 'n': 20, 'type': 'residential'},       # Altona
        {'lat': 53.5753, 'lon': 10.0153, 'n': 18, 'type': 'residential'},      # Eppendorf
        {'lat': 53.5580, 'lon': 10.0742, 'n': 15, 'type': 'business'},         # Wandsbek
        {'lat': 53.5252, 'lon': 9.9350, 'n': 12, 'type': 'residential'},       # Bahrenfeld
        {'lat': 53.5440, 'lon': 10.0465, 'n': 10, 'type': 'business'},         # HafenCity
    ]
    ...
    for cluster in clusters:
        for i in range(cluster['n']):
            ...
            # Premium customers (20% of orders) have stricter windows
            ...
            deliveries.append({
                ...
                'order_value': np.random.normal(35, 10) if not is_premium else np.random.normal(50, 15)
            })
            delivery_id += 1
...
deliveries_df = generate_hamburg_deliveries(100)
...
print(f"Premium customers: {deliveries_df['is_premium'].sum()}")
...
def visualize_deliveries(deliveries_df, depot_location, routes=None):
    ...
    plt.scatter(depot_location['lon'], depot_location['lat'],
                s=500, c='red', marker='s', zorder=5,
                label='Depot (Hauptbahnhof)', edgecolors='black', linewidths=2)

    # Plot deliveries by type
    regular = deliveries_df[~deliveries_df['is_premium']]
    premium = deliveries_df[deliveries_df['is_premium']]

    plt.scatter(regular['lon'], regular['lat'],
                s=100, c='#537E8F', alpha=0.6, label='Regular Delivery', zorder=3)
    plt.scatter(premium['lon'], premium['lat'],
                s=150, c='#D73502', marker='*', label='Premium Delivery', zorder=4)
...
We provide only basic helper functions. **You must implement the routing algorithms yourself.**
...
def create_distance_matrix(deliveries_df, depot_location):
    ...
    Returns numpy array where index 0 is depot, indices 1-100 are deliveries.
    """
...
def calculate_route_cost(route, deliveries_df, distance_matrix, depot_location):
    ...
    return {
        'total_cost': driver_cost + fuel_cost + penalty_cost,
...
You must develop a routing solution that assigns all 100 deliveries to 4 drivers and provides:
...
- **Problem understanding** (1 min): Eliza's pain points in your own words
- **Your approach** (2 min): Algorithm choice and justification
- **Results** (3 min): Routes visualization, cost savings, on-time improvement
- **Business impact** (1 min): Recommendations for QuickBite
- **Q&A preparation** (1 min): Anticipate questions about trade-offs
...
2. **Vehicle capacity:** Each driver can carry max 28 meals at once
...
4. **Respect premium customer time windows** when possible
:::

## Tips for Success

1. **Capacity matters:** With 100 deliveries and 28-meal capacity, you will need to use all drivers. This affects your algorithm design!
...
- `deliveries_df`: DataFrame with 100 deliveries
